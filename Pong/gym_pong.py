...
class PongScene(Scene):
    ...
    def global_step(self):
        ...
        if self.bounce_n_right > 1:  # hua-todo
            self.score_left += 1
            self.score_right -= 1
...
class RoboschoolPong(gym.Env, SharedMemoryClientEnv):
    ...
    def step(self, a):
        ...
        self.p0_rewards = self.scene.score_left
        self.p1_rewards = self.scene.score_right

        # return the results
        return state, sum(self.rewards), False, {'score_board': self.scene.score_board,
                                                 'p0_rewards': self.p0_rewards,
                                                 'p1_rewards': self.p1_rewards}
